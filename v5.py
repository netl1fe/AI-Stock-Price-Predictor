...
def fetch_macro_data(start_date, end_date):
    ...
    sp500 = pdr.get_data_yahoo('^GSPC', start=start_date, end=end_date)['Close'].pct_change()
    sp500.name = 'sp500'

    # Fetch VIX Close prices
    vix = pdr.get_data_yahoo('^VIX', start=start_date, end=end_date)['Close']
    vix.name = 'vix'

    return pd.concat([sp500, vix], axis=1)
...
data[['sp500', 'vix']] = data[['sp500', 'vix']].fillna(method='ffill')
data = data.dropna()
